...
- **Found translation key already mapped to a map or parent of new key already mapped to a string**: this happen in this kind of situation: `t('parent', {defaultValue: 'foo'})` and `t('parent.child', {defaultValue: 'bar'})`. `parent` is both a translation and an object for `child`.
